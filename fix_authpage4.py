...
if start_index == -1:
    start_index = content.find('          ) : (') # just fallback
    # Actually wait, let's just find the `) : (` that is followed by `<div className="space-y-4">`
    idx = content.find(') : (')
    while idx != -1:
        if 'className="space-y-4"' in content[idx:idx+50]:
            start_index = idx
            break
