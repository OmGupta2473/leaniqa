...
# Replace confirmMealMutation block using regex
import re
pattern = r"const confirmMealMutation = useMutation\(\{.*?\}\);(\s*)const handleSend"
new_mutation = """const confirmMealMutation = useMutation({
...
content = re.sub(pattern, new_mutation, content, flags=re.DOTALL)
